...
export default function ClientOffresPage() {
  ...
  if (viewMode === "list") {
    return (
      <div className="max-w-[1000px] mx-auto py-10 px-4 sm:px-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
        {/* Header */}
        <div className="mb-6">
          <PageHeader
            title="Offres envoyées"
            subtitle="Suivez et gérez les offres que vous avez envoyées aux freelances."
            actions={
              <Link
                href="/dashboard/client/candidatures"
                className="inline-flex items-center justify-center rounded-[10px] bg-[#2D5BE3] px-[18px] py-[10px] text-[13px] font-semibold text-white hover:bg-[#1F4DD4] transition-colors"
              >
                Candidatures
              </Link>
            }
        ...
        <div className="grid grid-cols-2 gap-4 mb-6 sm:grid-cols-4">
          <StatCard label="Total envoyées" value={stats.total} icon="📨" tone="blue" />
          ...
          <StatCard label="Clôturées" value={stats.refusees} icon="📁" tone="neutral" />
        </div>
