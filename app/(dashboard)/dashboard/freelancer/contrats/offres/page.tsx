...
// ── Design tokens — modèle : pipeline-candidat.html ─────────
const INK = "#14213D";
const PAPER = "#F5F6F4";
const LINE = "#DADFDD";
const GREEN = "#1F7A5C";
...
const AMBER = "#B8720A";
const AMBER_SOFT = "#FBEDD8";
const RED = "#B23A2E";
const RED_SOFT = "#F8E7E4";
const MUTED = "#6B7280";

const FONT_SERIF = "var(--font-fraunces, serif)";
const FONT_MONO = "var(--font-ibm-plex-mono, monospace)";
const FONT_SANS = "var(--font-space-grotesk, Inter, sans-serif)";
...
export default function FreelancerOffresPage() {
  ...
  if (viewMode === "list") {
    return (
      <div className="max-w-[1000px] mx-auto py-10 px-4 sm:px-8" style={{ fontFamily: FONT_SANS, color: INK }}>
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-[11px] uppercase tracking-[0.08em] font-medium mb-1" style={{ fontFamily: FONT_MONO, color: GREEN }}>
              Offres reçues
            </p>
            <h2 className="text-2xl font-medium" style={{ fontFamily: FONT_SERIF }}>Mes offres reçues</h2>
            <p className="text-sm mt-1" style={{ color: MUTED }}>
              Consultez et gérez les offres envoyées par les clients
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            { label: "Total reçues", value: stats.total, color: INK },
            { label: "En attente", value: stats.enAttente, color: AMBER },
            { label: "Acceptées", value: stats.acceptees, color: GREEN },
            { label: "Clôturées", value: stats.termine, color: MUTED },
          ].map((s) => (
            <div key={s.label} className="rounded-xl border p-4 text-center bg-white" style={{ borderColor: LINE }}>
